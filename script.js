...
// Function to fill the boardSquaresArray with initial piece positions
...
// Function to update the boardSquaresArray after a move
...
// Function to generate FEN (Forsyth-Edwards Notation) for the current board state
function generateFEN(boardSquares){
  ...
  while(rank>=1){
   ...
   // Replace consecutive "blank" occurrences with their count
  ...
  let castlingString="";
   ...
   // Determine castling possibilities for both colors
 ...
 // Build the castling string based on possibilities
...
function performCastling(
  ...
  //function for performing of EnPassant  
...
//fnction for selection of choices in case of promotion
...
//function for clearing the promotion options
...
//function for performing promotion
...
// function for possible chances for enpassant
...
//function for checking of pawn moves in forward direction
...
//function for setting up of the knight moves
...
//function for setting up of the moves of Rook
...
//function for setting up of the moves of Bishop 
...
//function for setting up of the moves of queen
...
function getKingMoves(startingSquareId, pieceColor, boardSquaresArray) {
  ...
  //function for setting up of shortcastling
...
function getPieceAtSquare(squareId, boardSquaresArray) {
  ...
  //function for checking wheather the king is in  check or not
...
//function for checking up of a valid move wheather the move is valid when the king is in check or not
...
//function for checking of checkmate and stalemate
...
function updateMoveHistory() {
  ...
  historyList.innerHTML = ""; // Clear previous moves

  for (let i = 0; i < moves.length; i++) {
    const move = moves[i];
    const listItem = document.createElement("li");
    listItem.textContent = `${move.pieceColor}'s ${move.pieceType} from ${move.from} to ${move.to}`;
    historyList.appendChild(listItem);
  }
}

// Modify your existing makeMove function to call updateMoveHistory
function makeMove(
  startingSquareId,
  destinationSquareId,
  pieceType,
